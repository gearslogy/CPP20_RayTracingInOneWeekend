...
#include "primitives/SdfMandelBulb.hpp"
...
int main() {
    ...
    Camera cam(lookfrom, lookat,0.2, 20, aspect_ratio, dist_to_focus-1 );
    ...
    HittableList world;
    auto groundObj = std::make_shared<Sphere>(Vec3(0.0f,-1000.5f,-1.0f), 1000.0);
    auto sphereObj = std::make_shared<Sphere>(Vec3(-2.0f,0,0), 0.5);
    auto lambertian_ground = std::make_shared<Lambertian>(Color(0.5,0.5,0.5));
    groundObj->setMaterial(lambertian_ground);
    sphereObj->setMaterial(lambertian_ground);
    auto mandelMat = std::make_shared<Lambertian>(Color(0.7,0.7,0.7));
    auto sdfSphere =  std::make_shared<SdfSphere>(Vec3{2,0,0},0.5);


    auto mandel =  std::make_shared<SdfMandleBulb>();

    mandel->setMaterial(mandelMat);
    sdfSphere->setMaterial(mandelMat);
    world.add(groundObj);
    world.add(sphereObj);
    world.add(sdfSphere);
    world.add(mandel);
