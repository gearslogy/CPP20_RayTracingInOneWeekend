...
void computeSamples(Vec3 &pixel_color, int i, int j, int image_width, int image_height, Camera &cam, HittableList & world, int max_depth){
    auto u =     (double(i) + random_double() ) / (image_width-1);
    auto v = 1-  ( double(j) + random_double()) / (image_height-1);    // left cornel is 0,0
    auto ray = cam.getRay(u,v);
    std::unique_lock lg(sampleLock);
